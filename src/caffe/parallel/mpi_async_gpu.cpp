...
#include "caffe/parallel/mpi_async_gpu.hpp"
...
class MPIAsyncGPU<Dtype>::Reducer : public InternalThread {
  public:
    MPIAsyncGPU<Dtype> *sync_;
    int tid_;
    Timer timer_queue_;
    ...
    Reducer(MPIAsyncGPU<Dtype> *sync, int tid)
        : sync_(sync), tid_(tid),
...
template<typename Dtype>
void MPIAsyncGPU<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
  for (int i=0; i<reducers.size(); ++i) {
...
template<typename Dtype>
void MPIAsyncGPU<Dtype>::allreduce() {
  DLOG(INFO) << "allreduce()";
}

template<typename Dtype>
void MPIAsyncGPU<Dtype>::allreduce(int param_id) {
  DLOG(INFO) << "allreduce(param_id)";
  param_solo_.push(param_id);
}

template<typename Dtype>
int MPIAsyncGPU<Dtype>::on_apply(int param_id) {
...
template<typename Dtype>
void MPIAsyncGPU<Dtype>::Step(int iters) {
