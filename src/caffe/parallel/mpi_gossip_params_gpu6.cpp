...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::next() {
  if (cube_) {
    if (rotate_) {
      next_cube_rotate();
    }
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::next_cube() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::next_cube_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::next_diffuse() {
  if (hci_ > logp_) {
    hci_ = 0;
  }
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::next_diffuse_rotate() {
  if (hci_ > logp_) {
    hci_ = 0;
    mci_ = (mci_+1)%comm_size_;
    comm_rank_ = caffe::mpi::comm_rank(comms_[mci_]);
...
template<typename Dtype>
MPIGossipParamsGPU6<Dtype>::MPIGossipParamsGPU6(
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::on_start() {
  DLOG(INFO) << "on_start()";
}

template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::on_begin() {
  DLOG(INFO) << "on_begin()";
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::make_progress() {
  CPUTimer timer;
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::after_forward() {
  DLOG(INFO) << "after_forward()";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::allreduce(int param_id) {
  DLOG(INFO) << "allreduce(param_id)";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::allreduce() {
  DLOG(INFO) << "allreduce()";
  make_progress();
}

template<typename Dtype>
int MPIGossipParamsGPU6<Dtype>::on_apply(int param_id) {
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::on_update() {
  DLOG(INFO) << "on_update()";
  make_progress();
}

template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::Run() {
...
template<typename Dtype>
void MPIGossipParamsGPU6<Dtype>::Step(int iters) {
...
INSTANTIATE_CLASS(MPIGossipParamsGPU6);
